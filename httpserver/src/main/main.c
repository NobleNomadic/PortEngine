...
int isPathSecure(const char *requestedPath, char *safePath, size_t safePathSize) {
  ...
  if (strstr(requestedPath, "//") != NULL ||
      ...
      strstr(requestedPath, "~") != NULL) {
    printf("\033[91m[!] Suspicious path blocked: %s\033[0m\n", requestedPath);
    return 0;
  }
